...
impl Okuchi {
    ...
    pub fn max_plaintext_bytes(pub_key: &PublicKey) -> usize {
    ...
    pub fn decrypt(priv_key: &PrivateKey, ciphertext: &Ciphertext) -> Result<Vec<u8>> {
        ...
        let l_cp = util::l_function(&cp, p);
        let l_gp = util::l_function(&priv_key.g_p_precomputed, p);
